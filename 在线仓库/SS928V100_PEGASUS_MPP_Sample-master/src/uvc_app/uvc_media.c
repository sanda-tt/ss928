...
static td_s32 sample_uvc_venc_init(uvc_media_cfg *media_cfg)
{
    ...
    ret = __sample_comm_venc_start_get_stream(&venc_chn, venc_chn_num);
    if (ret != TD_SUCCESS) {
...
static td_s32 sampe_uvc_start_vi_vpss_venc(uvc_media_cfg *media_cfg)
{
    ...
    if (is_need_venc == TD_TRUE) {
        ...
        ret = __sample_start_get_yuv(vpss_grp, vpss_chn);
    }

    return TD_SUCCESS;

venc_exit:
    __sample_comm_venc_stop_get_stream();
    sample_comm_venc_stop(venc_chn);
vi_unbind_vpss:
