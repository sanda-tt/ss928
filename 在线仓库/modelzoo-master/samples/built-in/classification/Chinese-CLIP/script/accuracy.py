...
def get_label_dict(label_path):
    ...
    with open(label_path, 'r')as f:
        for line in f.readlines():
            ...
            label_dict[key[0].split(".")[0]] = key[1]
            count+=1
    return label_dict
...
def get_accuracy(infer_path, json_file, label_dict, topk=5):
    ...
    if 'value' not in pre_json.keys():
        ...
        for i in range(topk):
            pre_json["value"].append({"key": "Top" + str(i + 1) + " accuracy", "value": str( round(accuracy[i] * 100, 2)) + '%'})
        writer = open(json_file, 'w')
        json.dump(pre_json, writer)
        writer.close()

'''
 path to output bin files 
 annotation files path, "val_label.txt"  
 metric result json file name
'''
