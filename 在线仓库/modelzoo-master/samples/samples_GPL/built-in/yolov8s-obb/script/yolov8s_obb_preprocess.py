...
def letterbox(img, target_size=(1024, 1024)):
    """保持比例缩放图片并填充至目标尺寸
    
    Args:
        img: 输入图像 (BGR格式)
        target_size: 目标尺寸 (height, width)，默认为 (1024, 1024)
...
def image_to_bin(image_path, output_bin_path, target_size=(1024, 1024)):
    """将图片转换为bin文件
    ...
    # 预处理图片
    processed_image = letterbox(image, target_size)
...
def process_images(file_list_path, output_dir, bin_list_file, target_size=(1024, 1024), max_count=5000):
    """处理file_list.json中的图片，转换为bin文件并保存到输出文件夹
    
    Args:
        file_list_path: 包含图片路径的JSON文件
        output_dir: 输出目录
        bin_list_file: 输出的bin文件列表
        target_size: 目标尺寸，默认为 (1024, 1024)
        max_count: 最大处理数量
    """
    # 如果目标目录已存在，先删除以清空旧数据
    ...
    with open(bin_list_file, 'w') as f_list:
        for filename in tqdm(process_files, desc="YOLOv8s-obb预处理进度"):
            # 检查文件是否存在
...
def main():
    # 解析命令行参数
    parser = argparse.ArgumentParser(description='YOLOv8s-obb图片预处理 (适配DOTAv1数据集, 1024x1024输入): 等比例缩放、填充并转换为bin文件')
    ...
    print(f"YOLOv8s-obb预处理完成，bin文件已保存至 {args.output_dir}")
