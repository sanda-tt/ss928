...
typedef enum {
    ...
    OT_VENC_GOP_MODE_SINGLE_SP   = 6,
    OT_VENC_GOP_MODE_BUTT,
} ot_venc_gop_mode;

typedef struct {
    td_s32 ip_qp_delta; /* RW; Range: [-10, 30]; Format: 0.0; Qp-delta of I-frame relative to P-frame */
} ot_venc_gop_normal_p;

typedef struct {
    td_u32 sp_interval; /* RW; Range: [0, 1) or (1, gop-1]; Format: 0.0; Interval of SP-frame */
    td_s32 sp_qp_delta; /* RW; Range: [-10, 30]; Format: 0.0; Qp-delta of SP-frame relative to P-frame */
    td_s32 ip_qp_delta; /* RW; Range: [-10, 30]; Format: 0.0; Qp-delta of I-frame relative to P-frame */
} ot_venc_gop_dual_p;

typedef struct {
    td_u32 bg_interval; /* RW; Range: [gop, 65536]; Format: 30.0; Interval of Long-term reference frame */
    td_s32 bg_qp_delta; /* RW; Range: [-10, 30]; Format: 0.0; Qp-delta of BG-frame relative to P-frame */
    td_s32 vi_qp_delta; /* RW; Range: [-10, 30]; Format: 0.0; Qp-delta of VI-frame relative to P-frame */
} ot_venc_gop_smart_p;
...
typedef struct {
    td_u32 b_frame_num; /* RW; Range: [1, 3]; Format: 1.0; Number of inserted B-frame */
    td_s32 b_qp_delta;  /* RW; Range: [-10, 30]; Format: 0.0; Qp-delta of B-frame relative to P-frame */
    td_s32 ip_qp_delta; /* RW; Range: [-10, 30]; Format: 0.0; Qp-delta of I-frame relative to P-frame */
...
typedef struct {
    td_bool deblur_en; /* RW;Range: [0, 1]; Format:1.0; Background deblur enable switch */
    td_bool deblur_adaptive_en; /* RW;Range: [0, 1]; Format:1.0; Background deblur adaptive switch */
} ot_venc_deblur_param;

typedef struct {
    td_u8 gain; /* RW;Range: [0, 15]; Format:8.0; Threshold parameter for detecting trailing regions close to moving
        objects, a smaller value increases the likelihood of being detected as a trailing region. */
    td_u8 offset; /* RW;Range: [0, 255]; Format:8.0; Threshold parameter for detecting trailing regions close to moving
        objects, a smaller value increases the likelihood of being detected as a trailing region. */
    td_u8 qp_delta; /* RW;Range: [0, 15]; Format:1.0; The reduction value of block-level QP for the trailing region
        close to moving objects. */
    td_u8 min_qp; /* RW;Range: [0, 51]; Format:0.0; Lower bound of block-level QP constraint for the trailing region
        close to moving objects */
    td_u8 max_qp; /* RW;Range: [0, 51]; Format:51.0; Upper bound of block-level QP constraint for the trailing region
        close to moving objects */
} ot_venc_deblur_near_bg;

typedef struct {
    td_u8 qp_delta; /* RW;Range: [0, 15]; Format:4.0; The reduction value of block-level QP for the trailing region
        far away from moving objects. */
    td_u8 min_qp; /* RW;Range: [0, 51]; Format:0.0; Lower bound of block-level QP constraint for the trailing region
        far away from moving objects */
    td_u8 max_qp; /* RW;Range: [0, 51]; Format:51.0; Upper bound of block-level QP constraint for the trailing region
        far away from moving objects */
...
typedef struct {
    td_bool near_bg_en; /* RW;Range: [0, 1]; Format:1.0; Detection enable switch of trailing region close to
        moving objects */
    td_bool far_bg_en; /* RW;Range: [0, 1]; Format:1.0; Detection enable switch of trailing region far away from
        moving objects */
