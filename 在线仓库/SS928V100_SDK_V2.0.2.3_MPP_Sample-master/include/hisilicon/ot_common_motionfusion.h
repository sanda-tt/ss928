...
#define OT_MFUSION_MAX_POSITION_NUM 128
...
typedef struct {
    td_u32 rep_num;  /* RW; data start position of invalid, in case of pts overlap */
    td_u32 data_num; /* RW; data length of the occupied buffer */
    ot_mfusion_sample_data data[OT_MFUSION_COMMON_BUF_SIZE];
} ot_mfusion_gyro_data;
...
typedef struct {
    td_bool enable;
    td_s32 drift[OT_MFUSION_AXIS_NUM]; /* RW; Range:[-0x80000000, 0x7FFFFFFF]; Zero drift of imu */
} ot_mfusion_drift;

typedef struct {
    td_bool enable;
    /* RW; Range:[-0x80000000, 0x7FFFFFFF]; value:0/32768/-32768; prec: 32768; Six side calibration matrix data */
...
typedef enum {
    OT_ROTATION_COMPENSATION_V1 = 0,    /* send quaternion type. */
    OT_ROTATION_COMPENSATION_V2 = 1,    /* send position data type. */
    OT_ROTATION_COMPENSATION_VERSION_BUTT
} ot_rotation_compensation_version;

typedef struct {
    td_u32 buf_size;
    ot_rotation_compensation_version version;
...
typedef struct {
    ...
    td_s32 z;
    td_u64 pts;
} ot_mfusion_position_sample_data;

typedef struct {
    td_u32 data_num;
    ot_mfusion_position_sample_data data[OT_MFUSION_MAX_POSITION_NUM];
} ot_mfusion_position_data;
