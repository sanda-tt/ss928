...
typedef enum {
    ...
    OT_DIS_MODE_HYBRID,              /* Use with gyro and gme in 6 dof. */
    OT_DIS_MODE_4_DOF_HYBRID,        /* Use with gyro and gme in 4 dof. */
    OT_DIS_MODE_BUTT,
} ot_dis_mode;

/* The motion level of camera */
typedef enum {
    OT_DIS_MOTION_LEVEL_LOW = 0,   /* Low motion level. Not supported. */
...
typedef struct {
    ...
    td_u32  moving_subject_level;    /* RW; Range:[0,7]; Moving Subject level. 7: GME off */
    ...
    td_u32  strength;                /* RW; Range:[0,1024]; The DIS strength for different light. */
...
typedef struct {
    ...
    td_u32              buf_num;               /* RW; Range:[5,10]; Buf num for GME DIS. */
...
typedef struct {
    td_u32 match_frame; /* RW; Range:[0, OT_VI_MAX_WDR_FRAME_NUM - 1]; only support wdr and gyro mode;
                           0: short frame, 1: median frame, 2: long frame */
} ot_dis_wdr_attr;
