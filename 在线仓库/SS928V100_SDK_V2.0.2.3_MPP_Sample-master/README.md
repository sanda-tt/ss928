# SS928V100_SDK_V2.0.2.3_MPP_Sample
...
```
SDK对应目录SS928V100_SDK_V2.0.2.3/smp/a55_linux/mpp/sample/
```

涉及删除的文件和目录：

```
src/aisr/aisr_model.bin
src/aisr/test01_QP30_cvt.yuv
src/aisr/test_in.yuv
src/avs/data目录
src/gfbg/res目录
...
src/sample_object_track/data目录                                   #源自SDK/SVP_PC/HiAI-SOT/sot_sample/sample_object_track
src/sample_object_track/model目录                                  #源自SDK/SVP_PC/HiAI-SOT/sot_sample/sample_object_track
src/svp/ovd/data目录                                               #源自SDK/SVP_PC/HiAI-OVD/ovd_sample/HiAI-OVD_board
src/svp/ovd/img目录                                                #源自SDK/SVP_PC/HiAI-OVD/ovd_sample/HiAI-OVD_board
src/svp/ovd/model目录                                              #源自SDK/SVP_PC/HiAI-OVD/ovd_sample/HiAI-OVD_board
src/svp/ovd_track/data/class_lists.txt                            #源自SDK/SVP_PC/HiAI-OVD/ovd_sample/HiAI-OVD_board/data
src/svp/ovd_track/data/dolls_video.h264                           #源自SDK/SVP_PC/HiAI-OVD/ovd_sample/HiAI-OVD_board/data
src/svp/ovd_track/data/text_features.bin                          #源自SDK/SVP_PC/HiAI-OVD/ovd_sample/HiAI-OVD_board/data
src/svp/ovd_track/model/hisi_ovd_v1.0.0_coco_46.1_lvis_30.3.om    #源自SDK/SVP_PC/HiAI-OVD/ovd_sample/HiAI-OVD_board/model
src/svp/ovd_track/model/HiAI-SOT-Base.om                          #源自SDK/SVP_PC/HiAI-SOT/sot_sample/sample_object_track/model
```
...
2. 运行sample_venc 0 0 0 X后，等待2~4秒，连按两次回车退出程序，查看当前目录是否生成 stream_chn0.h265、stream_chn1.h264 文件。

   ```bash
   ./sample_venc 0 0 0 X	# X 为 sensor 类型。
   ```

![image-20260422174103846](./README.assets/image-20260422174103846.png)

b.rtsp拉流测试

1. 拷贝sample_venc至板端。

2. 运行sample_venc 0 1 0 X后，使用VLC/PotPlayer等拉流软件进行拉流(拉流地址：rtsp://<板卡IP>:554/live0)。

   ```bash
   ./sample_venc 0 1 0 X    # X 为 sensor 类型。
   ```

![image-20260422174443211](./README.assets/image-20260422174443211.png)

![image-20260422174355988](./README.assets/image-20260422174355988.png)

#### 9). opencv 说明

##### ①. 编译说明

1. src/opencv下执行make, 生成sample_opencv。

##### ②. 运行说明

1. 依赖libopencv_world.so库，默认在/opt/lib下，已加载。
...
![image-20260422174812102](./README.assets/image-20260422174812102.png)

![image-20260422174820366](./README.assets/image-20260422174820366.png)
...
1. 编译Sample前，可根据实际使用的Sensor类型修改src/Makefile.param中的相关配置。若不修改，则默认使用src/Makefile.param中配置的Sensor类型;程序运行时也支持通过参数动态指定Sensor类型。
2. src/svp/ovd_sot下执行make, 生成sample_ovd_sot。
3. 共80类目标类别信息记录在/opt/sample/ovd_sot/data/class_lists.txt文件内。

##### ②. 使用说明

a.ovd目标检测测试

1. 拷贝sample_ovd_sot至板端。

2. 修改/opt/ko/load_ss928v100，取消ot_svp_npu.ko相关注释，注释ot_pqp.ko；重启或者remod ot_pqp后./load_ss928v100 -a重新加载ko。

   ![image-20260422180052943](./README.assets/image-20260422180052943.png)

   ![image-20260422180146873](./README.assets/image-20260422180146873.png)

3. 运行sample_ovd_sot 0 H X后，不同的类别不同的颜色的矩形框。

   ```bash
   ./sample_ovd_sot 0 H X    # H 为 实时流通路； X 为 sensor 类型。
   ```

   ![image-20260422180640839](./README.assets/image-20260422180640839.png)

   ![image-20260422180722156](./README.assets/image-20260422180722156.png)

   ![image-20260422180759147](./README.assets/image-20260422180759147.png)

   ![image-20260422180811100](./README.assets/image-20260422180811100.png)

b.ovd目标检测+sot单目标跟踪测试

1. 运行sample_ovd_sot 1 H X后，首先运行ovd目标检测，不同的类别不同的颜色的矩形框，最靠近中间区域的目标稳定且等待3S左右开启跟踪；此时运行sot单目标跟踪，矩形框颜色变黄，移动目标矩形框可跟踪移动。

2. 将跟踪目标移出摄像头范围后丢失跟踪目标切换运行ovd目标检测寻找新目标。

   ```bash
   ./sample_ovd_sot 1 H X    # H 为 实时流通路； X 为 sensor 类型。
   ```

   ![image-20260422181648794](./README.assets/image-20260422181648794.png)

   ![image-20260422180722156](./README.assets/image-20260422180722156.png)

   ![image-20260422181813653](./README.assets/image-20260422181813653.png)

   ![image-20260422181938081](./README.assets/image-20260422181938081.png)

   ![image-20260422180811100](./README.assets/image-20260422180811100.png)

   ![image-20260422182006179](./README.assets/image-20260422182006179.png)
