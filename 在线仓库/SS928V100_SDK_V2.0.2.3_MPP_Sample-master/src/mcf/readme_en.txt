...
2. Sample 2 and Sample 3 support HY_S0603 sensor. When using HY_S0603, because the sensor is not in slave mode, it cannot guarantee that the time of collecting 2 channels of data is completely synchronized, so the sample effect may not be correct.
...
5. The MCF maximum performance of SS927v100 can only reach 2 x 4M, so samples 2 to 5 do not support the HY_S0603 sensor.
