...
static td_s32 sample_comm_venc_h264_param_init(ot_venc_chn_attr *chn_attr, sample_comm_venc_chn_param *chn_param)
{
    ...
    chn_attr->venc_attr.h264_attr.frame_buf_ratio = SAMPLE_FRAME_BUF_RATIO_MAX;
    ...
    sample_comm_venc_chn_param *chn_param)
{
    ...
    chn_attr->venc_attr.h265_attr.frame_buf_ratio = SAMPLE_FRAME_BUF_RATIO_MAX;
    if (rc_mode == SAMPLE_RC_CBR) {
...
td_s32 sample_comm_request_idr(ot_venc_chn chn, td_bool instant)
{
    td_s32 ret = ss_mpi_venc_request_idr(chn, instant);
    if(ret != TD_SUCCESS) {
        sample_print("ss_mpi_venc_request_idr chn[%d] failed with %#x!\n", chn, ret);
        return TD_FAILURE;
    }
...
static td_s32 sample_comm_push_frame_to_rtsp(td_s32 index, sample_comm_venc_stream_proc_info *stream_proc_info,
    ot_venc_stream *stream, ot_venc_stream_buf_info *stream_buf_info, ot_payload_type *payload_type)
{
    td_u32 i;
    unsigned char *pstreamdata;
    td_u32 streamdata_length;
    td_s32 is_key;

    ot_unused(stream_buf_info);

    if (payload_type[index] == OT_PT_H265) {
        for (i = 0; i < stream->pack_cnt; i++) {
            if (stream->pack[i].data_type.h265_type == OT_VENC_H265_NALU_SEI) {
                continue;
            }
            pstreamdata = (unsigned char *)stream->pack[i].addr + stream->pack[i].offset;
            streamdata_length = stream->pack[i].len - stream->pack[i].offset;
            ...
            rtsp_session_push_frame(index, pstreamdata, streamdata_length, is_key);
        }
    } else if (payload_type[index] == OT_PT_H264) {
        for (i = 0; i < stream->pack_cnt; i++) {
            ...
            is_key = 0;
            if (stream->pack[i].data_type.h264_type == OT_VENC_H264_NALU_IDR_SLICE ||
                stream->pack[i].data_type.h264_type == OT_VENC_H264_NALU_I_SLICE) {
                is_key = 1;
            }
    ...
    td_s32 index, ot_venc_stream_buf_info *stream_buf_info, ot_payload_type *payload_type)
{
    ...
    if (stream_proc_info->push_rtsp) {
        /* step 2.5 : push frame to rtsp */
        ret = sample_comm_push_frame_to_rtsp(index, stream_proc_info, &stream, stream_buf_info, payload_type);
        if (ret != TD_SUCCESS) {
            return ret;
        }
    } else {
...
td_void *sample_comm_venc_get_venc_stream_proc(td_void *p)
{
    ...
    stream_proc_info.push_rtsp = para->push_rtsp;
...
td_void sample_comm_venc_set_push_rtsp(td_bool push_rtsp)
{
    g_para.push_rtsp = push_rtsp;
    sample_print("set push rtsp flag: %d!\n", push_rtsp);
}
