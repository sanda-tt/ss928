...
td_s32 scene_set_3dnr(ot_vpss_grp vpss_grp, const ot_scene_3dnr *_3dnr, td_u8 index)
{
    td_s32 ret;
    ...
    ret = ss_mpi_vpss_get_grp_param(vpss_grp, &nrx_attr);
    check_scene_ret(ret);
    ...
    ret = ss_mpi_vpss_set_grp_param(vpss_grp, &nrx_attr);
    check_scene_ret(ret);

    nrx_get_attr.nrx_param.nr_version = OT_VPSS_NR_V2;
    nrx_get_attr.nrx_param.nrx_norm_param_v2.op_mode = OT_OP_MODE_MANUAL;
    ret = ss_mpi_vpss_get_grp_param(vpss_grp, &nrx_get_attr);
    check_scene_ret(ret);

    static td_u32 count[OT_VPSS_MAX_GRP_NUM] = {0};
    if ((g_scene_pipe_param[index].module_state.debug == 1) && ((count[vpss_grp] % 15) == 0)) { /* 15 max count */
