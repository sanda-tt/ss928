...
td_void ot_scene_dynamic_nr_param_clip(ot_isp_nr_attr *nr_attr)
{
    td_u32 i;
...
td_s32 ot_scene_set_dynamic_linear_ca(ot_vi_pipe vi_pipe, td_u64 iso, td_u64 last_iso, td_u8 index)
{
    ...
    td_u32  iso_count = get_pipe_params()[index].dynamic_linear_ca.iso_count;
    td_u32 *iso_level_thresh = get_pipe_params()[index].dynamic_linear_ca.iso_level;
    td_u32 iso_level = (td_u32)scene_get_level_ltoh_u32((td_u32)iso, iso_count, iso_level_thresh);
    ...
    if ((iso_level == (iso_count - 1)) || (iso_level == 0)) {
        for (i = 0; i < OT_ISP_CA_YRATIO_LUT_LENGTH; i++) {
            ca_y_ratio_iso_interp_lut[i] = get_pipe_params()[index].dynamic_linear_ca.ca_y_ratio_iso_lut[iso_level][i];
        }
    } else {
        td_u32 mid_iso = (td_u32)iso;
        td_u32 left_iso = iso_level_thresh[iso_level - 1];
        ...
        for (i = 0; i < OT_ISP_CA_YRATIO_LUT_LENGTH; i++) {
            ca_y_ratio_iso_interp_lut[i] = scene_interpulate(mid_iso, left_iso,
                get_pipe_params()[index].dynamic_linear_ca.ca_y_ratio_iso_lut[iso_level - 1][i], right_iso,
                get_pipe_params()[index].dynamic_linear_ca.ca_y_ratio_iso_lut[iso_level][i]);
        }
    }
...
td_s32 ot_scene_set_dynamic_hnr(ot_vi_pipe vi_pipe, td_u64 iso, td_u64 last_iso, td_u8 index)
{
    ...
    isp_hnr_attr.normal_blend = get_pipe_params()[index].dynamic_hnr.blend_en;
    td_u32 iso_level;
    td_u32 iso_count = get_pipe_params()[index].dynamic_hnr.iso_count;
    td_u32 *iso_level_ltoh = get_pipe_params()[index].dynamic_hnr.iso_level_ltoh;
    if (isp_hnr_attr.op_type == 0) { /* op_type_auto */
        printf("invalid op_type %d, only manual is supported!\n", isp_hnr_attr.op_type);
        return TD_FAILURE;
    } else { /* op_type_manual */
        iso_level = (td_u32)scene_get_level_ltoh_u32((td_u32)iso, iso_count, iso_level_ltoh);
        ot_scenecomm_expr_true_return(iso_level >= OT_SCENE_ISO_STRENGTH_NUM, TD_FAILURE);
        if ((iso_level == 0) || (iso_level == iso_count - 1)) {
            isp_hnr_attr.manual_attr.param.sfs = get_pipe_params()[index].dynamic_hnr.manual_sfs[iso_level];
            isp_hnr_attr.manual_attr.param.tfs = get_pipe_params()[index].dynamic_hnr.manual_tfs[iso_level];
        } else {
            isp_hnr_attr.manual_attr.param.sfs = scene_interpulate(iso, iso_level_ltoh[iso_level - 1],
                get_pipe_params()[index].dynamic_hnr.manual_sfs[iso_level - 1], iso_level_ltoh[iso_level],
                get_pipe_params()[index].dynamic_hnr.manual_sfs[iso_level]);
            isp_hnr_attr.manual_attr.param.tfs = scene_interpulate(iso, iso_level_ltoh[iso_level - 1],
                get_pipe_params()[index].dynamic_hnr.manual_tfs[iso_level - 1], iso_level_ltoh[iso_level],
                get_pipe_params()[index].dynamic_hnr.manual_tfs[iso_level]);
        }
    }
...
td_s32 ot_scene_set_dynamic_fpn(ot_vi_pipe vi_pipe, td_u64 iso, td_u64 last_iso, td_u8 index)
{
    ...
    static td_bool last_iso_tmp[OT_VI_MAX_PIPE_NUM] = {0};
    ot_isp_exp_info exp_info;
    td_u32 iso_count = get_pipe_params()[index].dynamic_fpn.iso_count;
    td_u32 *iso_level_thresh = get_pipe_params()[index].dynamic_fpn.iso_thresh;
...
td_s32 ot_scene_set_dynamic_3dnr(ot_vpss_grp vpss_grp, td_u32 iso, td_u8 index)
{
    ...
    td_s32 ret = scene_set_3dnr(vpss_grp, &nrx_attr, index);
    check_scene_ret(ret);
    return TD_SUCCESS;
}
