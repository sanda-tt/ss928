...
static td_s32 scene_load_dynamic_linear_ca(const td_char *module, ot_scene_dynamic_linear_ca *dynamic_linear_ca)
{
    ot_scenecomm_check_pointer_return(dynamic_linear_ca, TD_FAILURE);
    td_s32 ret;
    td_s32 get_value;
    td_u32 i;
    td_char node[SCENE_INIPARAM_NODE_NAME_LEN] = { 0 };

    scene_load_int(module, "dynamic_linear_ca:iso_count", dynamic_linear_ca->iso_count, td_u32);
    scene_load_array(module, "dynamic_linear_ca:iso_level", dynamic_linear_ca->iso_level,
                     dynamic_linear_ca->iso_count, td_u32);

    for (i = 0; i < dynamic_linear_ca->iso_count && i < OT_SCENE_ISO_STRENGTH_NUM; i++) {
        snprintf_truncated_s(node, SCENE_INIPARAM_NODE_NAME_LEN, "dynamic_linear_ca:ca_y_ratio_lut_iso_%u", i);
        scene_load_array(module, node, dynamic_linear_ca->ca_y_ratio_iso_lut[i], OT_ISP_CA_YRATIO_LUT_LENGTH, td_u32);
    }

    scene_load_int(module, "dynamic_linear_ca:ratio_count", dynamic_linear_ca->ratio_count, td_u32);
    scene_load_array(module, "dynamic_linear_ca:ratio_level", dynamic_linear_ca->ratio_level,
                     dynamic_linear_ca->ratio_count, td_u32);
    scene_load_array(module, "dynamic_linear_ca:blend_weight", dynamic_linear_ca->blend_weight,
                     dynamic_linear_ca->ratio_count, td_u32);

    for (i = 0; i < dynamic_linear_ca->ratio_count && i <OT_SCENE_RATIO_STRENGTH_NUM; i++) {
        snprintf_truncated_s(node, SCENE_INIPARAM_NODE_NAME_LEN, "dynamic_linear_ca:ca_y_ratio_lut_ratio_%u", i);
        scene_load_array(module, node, dynamic_linear_ca->ca_y_ratio_ratio_lut[i], OT_ISP_CA_YRATIO_LUT_LENGTH, td_u32);
    }
...
static td_s32 scene_load_dynamic_hnr(const td_char *module, ot_scene_dynamic_hnr *dynamic_hnr)
{
    ...
    scene_load_int(module, "dynamic_hnr:blend_en", dynamic_hnr->blend_en, td_bool);
    scene_load_int(module, "dynamic_hnr:iso_count", dynamic_hnr->iso_count, td_u32);
    scene_load_array(module, "dynamic_hnr:iso_level_ltoh", dynamic_hnr->iso_level_ltoh, dynamic_hnr->iso_count, td_u32);
    scene_load_array(module, "dynamic_hnr:manual_sfs", dynamic_hnr->manual_sfs, dynamic_hnr->iso_count, td_u32);
    scene_load_array(module, "dynamic_hnr:manual_tfs", dynamic_hnr->manual_tfs, dynamic_hnr->iso_count, td_u32);
    return TD_SUCCESS;
}
...
static td_s32 scene_load_scene_param_part1(const td_char *module, ot_scene_pipe_param *scene_param)
{
    ...
    scene_load_module(scene_load_dynamic_linear_ca, module, &scene_param->dynamic_linear_ca);
