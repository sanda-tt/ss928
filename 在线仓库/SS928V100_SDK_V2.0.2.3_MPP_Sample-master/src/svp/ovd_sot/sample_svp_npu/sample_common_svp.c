...
static sample_sns_type g_sns_type[SENSOR_NUM] = {
    SENSOR0_TYPE,
    SENSOR1_TYPE,
    SENSOR2_TYPE,
    SENSOR3_TYPE
...
/*
 * @brief: 根据类别分配RGB888颜色值
 * @param: class_id 类别id
 * @return: 返回RGB888颜色值
 */
static td_u32 get_color_channels(td_u32 class_id) {
    if (class_id < 0) {
        class_id = 0;
    } else if (class_id > CLASS_NUM-1) {
        class_id = CLASS_NUM-1;
    }

    td_u8 step = 255 / CLASS_NUM;
    td_u8 r = (class_id * step * 1) & 0xFF;
    td_u8 g = (class_id * step * 2) & 0xFF;
    td_u8 b = (class_id * step * 3) & 0xFF;

    return 0x00000000 | (r << 16) | (g << 8) | b;
}

td_s32 sample_common_svp_vgs_fill_rect(const ot_video_frame_info *frame_info,
    ot_sample_svp_rect_info *rect, td_u32 color, td_bool track_enable)
{
    ...
    for (i = 0; i < rect->num; i++) {
        ...
        if (track_enable == TD_TRUE && rect->num == 1) {
            vgs_add_cover.quad.thick = 16;
            vgs_add_cover.color = color;
        } else {
            vgs_add_cover.quad.thick = OT_SAMPLE_IVE_DRAW_THICK;
            vgs_add_cover.color = get_color_channels(rect->rect[i].class_id);
        }
 ...
 * @return: 无
 */
td_void set_sns_type(sample_sns_type sns_type) {
    for(td_u8 i = 0; i < SENSOR_NUM; i++) {
        g_sns_type[i] = sns_type;
    }
}