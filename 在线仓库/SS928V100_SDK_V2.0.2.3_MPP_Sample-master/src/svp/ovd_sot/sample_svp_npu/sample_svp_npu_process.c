...
/*
 * @bref: 获取确定跟踪目标的中心点坐标，并根据中心点坐标判断是否启用跟踪
 * @param: ext_frame: 推理视频帧
 * @param: base_frame: 显示视频帧
 * @param: rect_info: 目标检测信息
 * @param: track_rect: 锁定跟踪目标信息
 * @param: track_enable: 是否启用跟踪
 * @return: 无
 */
static td_void get_track_fisrt_obj(const ot_video_frame_info *ext_frame, const ot_video_frame_info *base_frame,
    ot_sample_svp_rect_info *rect_info, ot_sample_svp_rect *track_rect, td_bool *track_enable)
{
    static ot_point closest_center_point = {0}, center_point = {0};
    static td_u64 stable_start_time = 0;
    static td_u64 drift_start_time = 0;
    static td_bool track_locked = TD_FALSE;
    td_u16 i, index = 0;
    td_u64 current_time;
    ot_point temp_point, distance_point = {0};
    td_s32 distance, min_distance;
    struct timespec ts;
    (td_void)clock_gettime(CLOCK_MONOTONIC, &ts);
    current_time = (td_u64)ts.tv_sec * 1000000000ULL + (td_u64)ts.tv_nsec;
    ...
    if (center_point.x == 0) {
        center_point.x = g_svp_npu_media_cfg.pic_size->width / 2;
        center_point.y = g_svp_npu_media_cfg.pic_size->height / 2;
    }
    min_distance = (td_s32)sqrt(pow(center_point.x, 2) + pow(center_point.y, 2));
    for (i = 0; i < rect_info->num; i++) {
        temp_point.x = (rect_info->rect[i].point[SAMPLE_SVP_NPU_RECT_RIGHT_TOP].x -
            rect_info->rect[i].point[SAMPLE_SVP_NPU_RECT_LEFT_TOP].x) / 2 +
            rect_info->rect[i].point[SAMPLE_SVP_NPU_RECT_LEFT_TOP].x;
        temp_point.y = (rect_info->rect[i].point[SAMPLE_SVP_NPU_RECT_LEFT_BOTTOM].y -
            rect_info->rect[i].point[SAMPLE_SVP_NPU_RECT_LEFT_TOP].y) / 2 +
            rect_info->rect[i].point[SAMPLE_SVP_NPU_RECT_LEFT_TOP].y;
        distance = (td_s32)sqrt(pow(temp_point.x - center_point.x, 2) +
            pow(temp_point.y - center_point.y, 2));
        if (distance < min_distance) {
            min_distance = distance;
            distance_point.x = temp_point.x;
            distance_point.y = temp_point.y;
            index = i;
        }
    }
    ...
    //判断是否启用跟踪
    if (track_locked) {
        if (*track_enable == TD_FALSE) {
            track_locked = TD_FALSE;
            stable_start_time = current_time;
            drift_start_time = current_time;
        } else if (abs(closest_center_point.x - distance_point.x) >= 100 ||
            abs(closest_center_point.y - distance_point.y) >= 100 ||
            rect_info->rect[index].class_id != track_rect->class_id) {
            if (drift_start_time == 0) {
                drift_start_time = current_time;
            }
            if ((current_time - drift_start_time) >= 1000000000ULL) {
                track_locked = TD_FALSE;
            }
            *track_enable = TD_FALSE;
        } else {
            drift_start_time = current_time;
            *track_enable = TD_TRUE;
        }
    } else if (abs(closest_center_point.x - distance_point.x) < 100 &&
        abs(closest_center_point.y - distance_point.y) < 100 &&
        rect_info->rect[index].class_id == track_rect->class_id) {
        ...
        if ((current_time - stable_start_time) >= 3000000000ULL) {
            track_locked = TD_TRUE;
            *track_enable = TD_TRUE;
        } else {
            *track_enable = TD_FALSE;
        }
    ...
    //坐标转换，更新跟踪目标矩形框信息
    closest_center_point.x = distance_point.x;
    closest_center_point.y = distance_point.y;
    memcpy_s(track_rect, sizeof(rect_info->rect[index]),
        &rect_info->rect[index], sizeof(rect_info->rect[index]));
    for (i = 0; i < 4; i++)
    {
        track_rect->point[i].x = (td_u32)((td_float)track_rect->point[i].x /
            base_frame->video_frame.width * ext_frame->video_frame.width) & (~1);
        track_rect->point[i].y = (td_u32)((td_float)track_rect->point[i].y /
            base_frame->video_frame.height * ext_frame->video_frame.height) & (~1);
    }
    return;
}

/*
 * @bref: 跟踪推理，并更新跟踪目标矩形框信息
 * @param: ext_frame: 推理视频帧信息
 * @param: base_frame: 显示视频帧信息
 * @param: trackHandle: 跟踪句柄
 * @param: rect_info: 目标检测信息
 * @param: track_rect: 锁定跟踪目标信息
 * @param: track_enable: 是否启用跟踪
 * @return: 返回成功或失败
 */
static td_s32 sot_acl_frame_proc(ot_video_frame_info *ext_frame, ot_video_frame_info *base_frame,
    td_u32 trackHandle, ot_sample_svp_rect_info *rect_info, ot_sample_svp_rect *track_rect, td_bool *track_enable)
{
    static td_bool fisrt_execute = TD_FALSE;
    td_u8 i;
    td_s32 ret = TD_FAILURE;
    OT_IAA_AlgMemInfo mem;
    OT_OBJECTTRACK_BoxParam seqBox;
    ...
    if (fisrt_execute == TD_FALSE) {
        memcpy_s(rect_info, sizeof(ot_sample_svp_rect_info),
            0, sizeof(ot_sample_svp_rect_info));
        rect_info->num = 1;
        memcpy_s(&rect_info->rect[0], sizeof(ot_sample_svp_rect),
            track_rect, sizeof(ot_sample_svp_rect));
        seqBox.x = rect_info->rect[0].point[SAMPLE_SVP_NPU_RECT_LEFT_TOP].x;
        seqBox.y = rect_info->rect[0].point[SAMPLE_SVP_NPU_RECT_LEFT_TOP].y;
        seqBox.width = rect_info->rect[0].point[SAMPLE_SVP_NPU_RECT_RIGHT_TOP].x -
            rect_info->rect[0].point[SAMPLE_SVP_NPU_RECT_LEFT_TOP].x;
        seqBox.height = rect_info->rect[0].point[SAMPLE_SVP_NPU_RECT_LEFT_BOTTOM].y -
            rect_info->rect[0].point[SAMPLE_SVP_NPU_RECT_LEFT_TOP].y;
        ret = AlgObjectTrack_ReadPreProcessInputData(&mem, ext_frame, virt_addr);
        OT_SOT_LOG_AND_GOTO_IF_EXPR_FALSE(ret == TD_SUCCESS, FAIL, "AlgObjectTrack_ReadPreProcessInputData");
        ret = AlgObjectTrackSetDebugInfo(trackHandle);
        OT_SOT_LOG_AND_GOTO_IF_EXPR_FALSE(ret == TD_SUCCESS, FAIL, "AlgObjectTrackSetDebugInfo");
        ret = SS_OBJECTTRACK_PreExecute(trackHandle, &seqBox, &mem);
        OT_SOT_LOG_AND_GOTO_IF_EXPR_FALSE(ret == TD_SUCCESS, FAIL, "SS_OBJECTTRACK_PreExecute");
        fisrt_execute = TD_TRUE;
    }

    ret = AlgObjectTrack_ReadPreProcessInputData(&mem, ext_frame, virt_addr);
    OT_SOT_LOG_AND_GOTO_IF_EXPR_FALSE(ret == TD_SUCCESS, FAIL, "AlgObjectTrack_ReadPreProcessInputData");
    ret = SS_OBJECTTRACK_Execute(trackHandle, &mem, &trackRes);
    OT_SOT_LOG_AND_GOTO_IF_EXPR_FALSE(ret == TD_SUCCESS, FAIL, "SS_OBJECTTRACK_Execute");
    rect_info->rect[0].point[SAMPLE_SVP_NPU_RECT_LEFT_TOP].x = trackRes.boxOut.x & (~1);
    rect_info->rect[0].point[SAMPLE_SVP_NPU_RECT_LEFT_TOP].y = trackRes.boxOut.y & (~1);
    rect_info->rect[0].point[SAMPLE_SVP_NPU_RECT_RIGHT_TOP].x = (trackRes.boxOut.x + trackRes.boxOut.width) & (~1);
    rect_info->rect[0].point[SAMPLE_SVP_NPU_RECT_RIGHT_TOP].y = trackRes.boxOut.y & (~1);
    rect_info->rect[0].point[SAMPLE_SVP_NPU_RECT_RIGHT_BOTTOM].x = (trackRes.boxOut.x + trackRes.boxOut.width) & (~1);
    rect_info->rect[0].point[SAMPLE_SVP_NPU_RECT_RIGHT_BOTTOM].y = (trackRes.boxOut.y + trackRes.boxOut.height) & (~1);
    rect_info->rect[0].point[SAMPLE_SVP_NPU_RECT_LEFT_BOTTOM].x = trackRes.boxOut.x & (~1);
    rect_info->rect[0].point[SAMPLE_SVP_NPU_RECT_LEFT_BOTTOM].y = (trackRes.boxOut.y + trackRes.boxOut.height) & (~1);
    for (i = 0; i < 4; i++) {
        rect_info->rect[0].point[i].x = (td_u32)((td_float)rect_info->rect[0].point[i].x /
            ext_frame->video_frame.width * base_frame->video_frame.width) & (~1);
        rect_info->rect[0].point[i].y = (td_u32)((td_float)rect_info->rect[0].point[i].y /
            ext_frame->video_frame.height * base_frame->video_frame.height) & (~1);
    }

    printf("trackRes.score = %f\n", trackRes.score);
    if (trackRes.score < 0.015) {
        *track_enable = TD_FALSE;
        fisrt_execute = TD_FALSE;
    }
    return ret;
FAIL:
    (td_void)AlgObjectTrackGetTimeCost(trackHandle);
...
static td_void *sample_svp_npu_ovd_acl_vdec_to_vo(td_void *args)
{
    ...
    while (g_svp_npu_thread_stop == TD_FALSE) {
        ...
        ret = sample_common_svp_vgs_fill_rect(&base_frame, sample_svp_npu_get_svp_rect_info(), SAMPLE_SVP_NPU_RECT_COLOR, track_enable);
        sample_svp_check_exps_goto(ret != TD_SUCCESS, base_release, SAMPLE_SVP_ERR_LEVEL_ERROR,
            "Error(%#x),sample_common_svp_vgs_fill_rect failed!\n", ret);
...
static td_void *sample_svp_npu_ovd_track_acl_vdec_to_vo(td_void *args)
{
    ot_video_frame_info base_frame, ext_frame;
    const td_s32 milli_sec = SAMPLE_SVP_NPU_MILLIC_SEC;
    const ot_vo_layer vo_layer = 0;
    ...
    while (g_svp_npu_thread_stop == TD_FALSE) {
        ...
        if (track_enable == TD_FALSE) {
            ...
            sample_svp_check_exps_goto(ret != TD_SUCCESS, base_release, SAMPLE_SVP_ERR_LEVEL_ERROR,
            "Error(%#x),sample_svp_npu_ovd_acl_frame_proc failed!\n", ret);
            get_track_fisrt_obj(&ext_frame, &base_frame, sample_svp_npu_get_svp_rect_info(),
                &track_rect, &track_enable);
        } else if (track_enable == TD_TRUE) {
            ret= sot_acl_frame_proc(&ext_frame, &base_frame, g_trackHandle, sample_svp_npu_get_svp_rect_info(),
                &track_rect, &track_enable);
            sample_svp_check_exps_goto(ret != TD_SUCCESS, base_release, SAMPLE_SVP_ERR_LEVEL_ERROR,
                "Error(%#x),sot_acl_frame_proc failed!\n", ret);
        }
...
td_void sample_svp_npu_acl_ovd_sot_board(td_u32 is_file)
{
    td_s32 ret;
    const td_u32 model_idx = 0;
    sample_svp_npu_thread_args args = {0};
    OT_IAA_AlgMemInfo track_model;
    ...
    if (g_svp_npu_terminate_signal == TD_FALSE) {
        ...
        const td_char *om_model_path = "./model/hisi_ovd_v1.0.0_coco_46.1_lvis_30.3.om";
        sample_svp_check_exps_goto(realpath(om_model_path, path) == TD_NULL, process_end0, SAMPLE_SVP_ERR_LEVEL_ERROR,
            "Error sample_svp_npu_acl_ovd_sot_board, Invalid %s file or path!\n", om_model_path);
        ret = sample_common_svp_npu_load_model(path, model_idx, TD_FALSE);
        sample_svp_check_exps_goto(ret != TD_SUCCESS, process_end0, SAMPLE_SVP_ERR_LEVEL_ERROR, "load model failed!\n");
        ...
        ret = AlgInitTrack(&track_model, &g_trackHandle);
        sample_svp_check_exps_goto(ret != TD_SUCCESS, track_model_fail, SAMPLE_SVP_ERR_LEVEL_ERROR,
            "Error(%#x),AlgInitTrack failed!\n", ret);
    }
    ...
    if (g_svp_npu_terminate_signal == TD_FALSE) {
        ...
        g_svp_npu_thread_stop = TD_FALSE;
        ret = pthread_create(&g_svp_npu_thread, 0, sample_svp_npu_ovd_track_acl_vdec_to_vo, (td_void*)&args);
        sample_svp_check_exps_goto(ret != 0, process_end4, SAMPLE_SVP_ERR_LEVEL_ERROR, "create thread failed!\n");
...
process_end3:
    (td_void)sample_svp_npu_acl_deinit_task(1, 0);
track_model_fail:
    (td_void)AlgObjectTrackGetTimeCost(g_trackHandle);
    (td_void)SS_OBJECTTRACK_DestroyTrackHdl(g_trackHandle);
    (td_void)SS_OBJECTTRACK_Deinit();
    (td_void)ss_mpi_sys_mmz_free(track_model.physAddr, track_model.virtAddr);
process_end2:
    if (is_file) {
        (td_void)sample_common_svp_destroy_vb_stop_vdec_vpss_vo(&g_svp_npu_vdec_param, &g_svp_npu_vdec_thread,
            &g_svp_npu_media_cfg, &g_svp_npu_vo_cfg);
...
td_void sample_svp_npu_acl_ovd(td_u32 is_file)
{
    switch (is_file) {
        case 0:
        case 1:
...
/* function : show the sample of ovd sot*/
td_void sample_svp_npu_acl_ovd_sot(td_u32 is_file)
{
    switch (is_file) {
        case 0:
        case 1:
            sample_svp_npu_acl_ovd_sot_board(is_file);
            break;
        default:
            break;
    }
