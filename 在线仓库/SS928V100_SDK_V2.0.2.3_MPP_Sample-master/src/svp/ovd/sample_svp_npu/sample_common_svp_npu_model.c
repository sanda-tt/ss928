    ...
    td_bool is_input, td_u32 *total_line_num, td_u32 *line_byte_num)
{
    ...
    *total_line_num = 1;
    ...
    if (is_input == TD_TRUE) {
        ...
        if (aipp_info.input_format == SVP_ACL_YUV420SP_U8 || aipp_info.input_format == SVP_ACL_YVU420SP_U8) {
    ...
    ot_size *pic_size)
{
    ...
    sample_svp_check_exps_return(ret != SVP_ACL_SUCCESS, TD_FAILURE, SAMPLE_SVP_ERR_LEVEL_ERROR,
        "get num dims failed!\n");
    pic_size->height = dims.dims[SAMPLE_SVP_NPU_H_DIM_IDX]; // NCHW
    pic_size->width = dims.dims[SAMPLE_SVP_NPU_W_DIM_IDX]; // NCHW
    return TD_SUCCESS;
}
