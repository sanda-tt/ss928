...
#endif
#define UAC_ENVENT_PRINT_SUPPORT 1

#if UAC_ENVENT_PRINT_SUPPORT
typedef enum {
    UAC_EVENT_UNKNOEWN = 0,
    UAC_EVENT_CAPTURE_MUTE,
    UAC_EVENT_PLAYBACK_MUTE,
    UAC_EVENT_CAPTURE_VOLUME,
    UAC_EVENT_PLAYBACK_VOLUME,
    UAC_EVENT_CAPTURE_SRATE,
    UAC_EVENT_PLAYBACK_SRATE,
    UAC_EVENT_CAPTURE_STATE,
    UAC_EVENT_PLAYBACK_STATE,
} uac_ctrl_index;
#endif
...
#if UAC_ENVENT_PRINT_SUPPORT
static sample_uac_envent g_sample_uac_envent = {0};
#endif
...
#if UAC_ENVENT_PRINT_SUPPORT
static void show_control_detail(char *pre, snd_hctl_elem_t *h_elem)
{
    char *str;
    snd_ctl_elem_id_t *id;

    snd_ctl_elem_id_alloca(&id);
    snd_hctl_elem_get_id(h_elem, id);
...
static void hand_event_info(snd_hctl_elem_t *h_elem)
{
    show_control_detail("event info: ", h_elem);
}

static void hand_event_value(snd_hctl_elem_t *h_elem)
{
    int ret;
    int fu_value, rate, state;
    const char *name;
    unsigned int index;
    snd_ctl_elem_value_t *value;
    snd_ctl_elem_value_alloca(&value);
    ret = snd_hctl_elem_read(h_elem, value);
    if (ret < 0) {
        printf("[Func]:%s [Line]:%d info error: Control %s element read error: %s\n",
            __FUNCTION__, __LINE__, PCM_DEVICE_NAME, snd_strerror(ret));
        return;
    }

    name = snd_ctl_elem_value_get_name(value);
    index = snd_ctl_elem_value_get_index(value);
    switch (index) {
        case UAC_EVENT_CAPTURE_MUTE:
        case UAC_EVENT_PLAYBACK_MUTE:
            fu_value = snd_ctl_elem_value_get_boolean(value, 0);
            printf("ctrl_name:'%s' value:%d event:[%s]\n", name, fu_value, fu_value ? "mute" : "unmute");
            break;

        case UAC_EVENT_CAPTURE_VOLUME:
        case UAC_EVENT_PLAYBACK_VOLUME:
            fu_value = snd_ctl_elem_value_get_integer(value, 0);
            printf("ctrl_name:'%s' value:%d event:[%s]\n", name, fu_value, "volume");
            break;

        case UAC_EVENT_CAPTURE_SRATE:
        case UAC_EVENT_PLAYBACK_SRATE:
            rate = snd_ctl_elem_value_get_integer(value, 0);
            printf("ctrl_name:'%s' value:%d event:[%s]\n", name, rate, "rate");
            break;

        case UAC_EVENT_CAPTURE_STATE:
        case UAC_EVENT_PLAYBACK_STATE:
            state = snd_ctl_elem_value_get_boolean(value, 0);
            printf("ctrl_name:'%s' value:%d event:[%s]\n", name, state, state ? "start" : "stop");
            break;

        default:
            printf("ctrl_name:'%s' Unregistered event!!!! \n", name);
            break;
    }
}

static void hand_event_remove(snd_hctl_elem_t *h_elem)
{
    show_control_detail("event remove: ", h_elem);
}

static int goto_wait_events()
{
    /*
     * 为了防止无事件时阻塞在snd_hctl_handle_events函数中，
     * 而导致sample_audio_uac_envent_priuac_envent_pri线程不能正常退出，这里需要返回负值。
     */
    return -EAGAIN;
}

static int hand_element_events(snd_hctl_elem_t *elem, unsigned int mask)
{
    if (mask == SND_CTL_EVENT_MASK_REMOVE) {
        hand_event_remove(elem);
        return 0;
    }
    if (mask & SND_CTL_EVENT_MASK_INFO)
        hand_event_info(elem);
    if (mask & SND_CTL_EVENT_MASK_VALUE)
        hand_event_value(elem);
    return goto_wait_events();
}

static void hand_event_add(snd_hctl_elem_t *h_elem)
{
    snd_hctl_elem_set_callback(h_elem, hand_element_events);
    show_control_detail("event add: ", h_elem);
}

static int hand_hctl_events(snd_hctl_t *ctl ATTRIBUTE_UNUSED, unsigned int mask,
...
void *sample_audio_uac_envent_pri(void *arg)
{
    td_s32 ret;
    snd_hctl_t *handle = TD_NULL;
    sample_uac_envent *uac_envent_ctrl = (sample_uac_envent *)arg;

    ret = snd_hctl_open(&handle, PCM_DEVICE_NAME, 0);
    if (ret < 0) {
        printf("[Func]:%s [Line]:%d info error: Control %s open error: %s\n",
            __FUNCTION__, __LINE__, PCM_DEVICE_NAME, snd_strerror(ret));
        goto open_fail;
    }

    snd_hctl_set_callback(handle, hand_hctl_events);
    ret = snd_hctl_load(handle);
    if (ret < 0) {
        printf("[Func]:%s [Line]:%d info error: Control %s load error: %s\n",
            __FUNCTION__, __LINE__, PCM_DEVICE_NAME, snd_strerror(ret));
        goto load_fail;
    }

    while (uac_envent_ctrl->started) {
#ifndef __LITEOS__
        if (sample_uvc_get_quit_flag() != 0) {
            break;
        }
#endif
        int res = snd_hctl_wait(handle, PCM_WAIT_TIME_MS);
        if (res > 0) {
            snd_hctl_handle_events(handle);
        } else if (res == 0) {
            // timeout, do nothing
        } else {
            printf("[Func]:%s [Line]:%d info error: snd_hctl_wait ERR: %s (%d)\n",
                __FUNCTION__, __LINE__, snd_strerror(res), res);
        }
    }
load_fail:
...
open_fail:
    uac_envent_ctrl->started = TD_FALSE;

    printf("[Func]:%s [Line]:%d thread exit!\n", __FUNCTION__, __LINE__);

    return TD_NULL;
}
#endif /* end of #if UAC_ENVENT_PRINT_SUPPORT */
...
#if UAC_ENVENT_PRINT_SUPPORT
/* function : Create the thread to print uac envent */
static td_s32 sample_audio_create_trd_uac_envent_pri(td_void)
{
    td_s32 ret;
    sample_uac_envent *uac_envent_ctrl = &g_sample_uac_envent;

    uac_envent_ctrl->started = TD_TRUE;
    ret = pthread_create(&uac_envent_ctrl->pid, 0, sample_audio_uac_envent_pri, uac_envent_ctrl);
    if (ret != 0) {
        printf("[Func]:%s [Line]:%d info error: create pthread failed with 0x%x!\n", __FUNCTION__, __LINE__, ret);
        return TD_FAILURE;
    }
...
/* function : Destroy the thread to print uac envent */
static td_void sample_audio_destroy_trd_uac_envent_pri(td_void)
{
    sample_uac_envent *uac_envent_ctrl = &g_sample_uac_envent;

    if (uac_envent_ctrl->started) {
        uac_envent_ctrl->started = TD_FALSE;
        pthread_join(uac_envent_ctrl->pid, 0);
    }
}
#endif /* end of #if UAC_ENVENT_PRINT_SUPPORT */
...
static td_s32 sample_audio_create_uac_envent_pri_thread(td_void)
{
#if UAC_ENVENT_PRINT_SUPPORT
    return sample_audio_create_trd_uac_envent_pri();
#else
    return TD_SUCCESS;
#endif
}
...
static td_void sample_audio_destroy_uac_receive_thread(const uac_startup_param *param)
{
#if UAC_CAPTURE_SUPPORT
    return sample_audio_destroy_trd_uac_capture(param->ao_dev, param->ao_chn);
#else
    ot_unused(param);
#endif
}
...
static td_s32 sample_audio_create_thread(const uac_startup_param *param)
{
    ...
    if (ret != TD_SUCCESS) {
        sample_audio_debug(ret);
        goto create_receive_fail;
    }

    /* create uac event print thread */
    ret = sample_audio_create_uac_envent_pri_thread();
    if (ret != TD_SUCCESS) {
        sample_audio_debug(ret);
        goto create_envent_fail;
    }

    return TD_SUCCESS;

create_envent_fail:
    sample_audio_destroy_uac_receive_thread(param);
...
#if UAC_ENVENT_PRINT_SUPPORT
    sample_audio_destroy_trd_uac_envent_pri();
#endif
