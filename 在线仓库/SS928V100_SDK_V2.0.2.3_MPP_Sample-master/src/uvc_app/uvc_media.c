...
static ot_uvc_data_input_mode g_data_input_mode[OT_UVC_MAX_CHN_NUM] = {OT_UVC_MPP_BIND_UVC, OT_UVC_MPP_BIND_UVC};

ot_uvc_data_input_mode sample_uvc_get_input_mode(td_u32 dev_no)
{
    if ((ot_uvc_chn)dev_no >= OT_UVC_MAX_CHN_NUM) {
        return OT_UVC_DATA_INPUT_MODE_BUTT;
    }
    return g_data_input_mode[dev_no];
}

td_s32 sample_uvc_set_input_mode(td_u32 dev_no, ot_uvc_data_input_mode mode)
{
    if (mode >= 0 && mode <= OT_UVC_DATA_INPUT_MODE_BUTT && (ot_uvc_chn)dev_no < OT_UVC_MAX_CHN_NUM) {
        g_data_input_mode[dev_no] = mode;
        return TD_SUCCESS;
    }
    return TD_FAILURE;
}
...
{
    if (g_encoder_property[uvc_chn].format == VIDEO_IMG_FORMAT_MJPEG) {
        uvc_chn_attr->uvc_format = OT_UVC_FORMAT_MJPEG;
    } else if (g_encoder_property[uvc_chn].format == VIDEO_IMG_FORMAT_H264) {
        uvc_chn_attr->uvc_format = OT_UVC_FORMAT_H264;
    } else if (g_encoder_property[uvc_chn].format == VIDEO_IMG_FORMAT_H265) {
        uvc_chn_attr->uvc_format = OT_UVC_FORMAT_H265;
    } else if (g_encoder_property[uvc_chn].format == VIDEO_IMG_FORMAT_NV12) {
        uvc_chn_attr->uvc_format = OT_UVC_FORMAT_NV12;
    } else if (g_encoder_property[uvc_chn].format == VIDEO_IMG_FORMAT_NV21) {
        uvc_chn_attr->uvc_format = OT_UVC_FORMAT_NV21;
    } else if (g_encoder_property[uvc_chn].format == VIDEO_IMG_FORMAT_YUYV) {
...
static td_s32 sample_uvc_chn_init(ot_uvc_chn uvc_chn)
{
    ...
    ot_uvc_chn_attr_ex uvc_chn_attr_ex = {0};

    sample_uvc_get_uvc_chn_attr(uvc_chn, &uvc_chn_attr);
    if (g_data_input_mode[uvc_chn] != OT_UVC_SEND_USER_STREAM ||
        !sample_uvc_is_need_venc(g_encoder_property[uvc_chn].format)) {
        ret = ss_mpi_uvc_create_chn(uvc_chn, &uvc_chn_attr);
    } else { /* SEND_USER_STREAM  &&  is_need_venc */
        uvc_chn_attr_ex.uvc_format = uvc_chn_attr.uvc_format;
        uvc_chn_attr_ex.data_mode = OT_UVC_SEND_USER_STREAM;
        uvc_chn_attr_ex.buffer_size = 4096; /* 4096 KB for UVC user stream ring buffer */
        ret = ss_mpi_uvc_create_chn_ex(uvc_chn, &uvc_chn_attr_ex);
    }
    if (ret != TD_SUCCESS) {
...
static td_void sample_uvc_get_default_vpss_ext_chn_attr(td_u32 dev_no, ot_vpss_ext_chn_attr *vpss_ext_chn_attr)
{
    ...
    if (g_data_input_mode[dev_no] == OT_UVC_SEND_YUV_FRAME &&
        g_encoder_property[dev_no].format == VIDEO_IMG_FORMAT_YUYV) {
...
static td_s32 sample_uvc_vpss_init(uvc_media_cfg *media_cfg)
{
    ...
    if (g_encoder_property[media_cfg->dev_no].format == VIDEO_IMG_FORMAT_YUYV) {
        sample_uvc_get_default_vpss_ext_chn_attr(media_cfg->dev_no, &vpss_ext_chn_attr);
...
static td_s32 sample_uvc_venc_init(uvc_media_cfg *media_cfg)
{
    ...
    if (media_cfg->venc_chn_param.type != OT_PT_H264 && media_cfg->venc_chn_param.type != OT_PT_H265) {
        return TD_SUCCESS;
    }
    ot_venc_chn_attr venc_chn_attr;
    ret = ss_mpi_venc_get_chn_attr(venc_chn, &venc_chn_attr);
    if (ret != TD_SUCCESS) {
        sample_print("venc_get_chn_attr failed %#x\n", ret);
        return TD_SUCCESS;
    }
    if (venc_chn_attr.venc_attr.is_by_frame == TD_TRUE) {
        sample_print("disable venc slice\n");
        return TD_SUCCESS;
    }

    sample_print("enable venc slice\n");
    ot_venc_slice_split slice_cfg;
    td_u32 height = media_cfg->venc_chn_param.venc_size.height;
    td_u32 mbl_lcu = 16; /* 16 is H264 microblock lines */
    td_u32 slice_cnt = 4;
    if (media_cfg->venc_chn_param.type == OT_PT_H265) {
        mbl_lcu = 32; /* 32 is H265 LCU */
    }

    slice_cfg.enable = TD_TRUE;
    slice_cfg.split_mode = 1;
    slice_cfg.split_size = (height + mbl_lcu * slice_cnt - 1) / (mbl_lcu * slice_cnt);
    slice_cfg.slice_output_en = TD_TRUE;
    ret = ss_mpi_venc_set_slice_split(venc_chn, &slice_cfg);
    if (ret != TD_SUCCESS) {
        sample_print("set venc slice split failed %#x\n", ret);
        return TD_SUCCESS;
    }
...
{
    ot_uvc_chn    uvc_chn = (ot_uvc_chn)media_cfg->dev_no;
    ...
    if (sample_uvc_is_need_venc(fcc_format) == TD_TRUE) {
        ...
        if (g_data_input_mode[uvc_chn] == OT_UVC_MPP_BIND_UVC) {
        ...
        if (g_data_input_mode[uvc_chn] != OT_UVC_SEND_YUV_FRAME) {
            ot_vpss_chn chn = (fcc_format == VIDEO_IMG_FORMAT_YUYV) ? media_cfg->vpss_ext_chn : media_cfg->vpss_chn;
            ret = sample_comm_vpss_bind_uvc(media_cfg->vpss_grp, chn, uvc_chn);
            if (ret != TD_SUCCESS) {
                goto uvc_exit;
            }
        }
...
static td_s32 sample_uvc_media_route(td_u32 dev_no)
{
    ...
    if (g_data_input_mode[dev_no] == OT_UVC_SEND_YUV_FRAME &&
        sample_uvc_is_need_venc(g_encoder_property[dev_no].format)) {
        g_data_input_mode[dev_no] = OT_UVC_SEND_VENC_STREAM;
    } else if (g_data_input_mode[dev_no] == OT_UVC_SEND_VENC_STREAM &&
        !sample_uvc_is_need_venc(g_encoder_property[dev_no].format)) {
        g_data_input_mode[dev_no] = OT_UVC_SEND_YUV_FRAME;
    }
...
td_void sample_uvc_send_user_stream(ot_uvc_chn uvc_chn, ot_venc_stream *venc_stream)
{
    td_s32 ret;
    ...
    for (td_u32 i = 0; i < venc_stream->pack_cnt; i++) {
        ...
        ret = ss_mpi_uvc_send_user_stream(uvc_chn, &user_stream, 0); /* replace stream addr with your own buffer */
        if (ret != 0) {
...
td_s32 sample_send_yuv_frame(td_u32 dev_no)
{
    ...
    ret = ss_mpi_uvc_send_frame((ot_uvc_chn)dev_no, &frame_info, 0);
...
td_s32 sample_venc_get_uvc_send(td_u32 dev_no)
{
    ...
    if (g_is_media_start[dev_no] == TD_TRUE && g_data_input_mode[dev_no] == OT_UVC_SEND_YUV_FRAME
        && !sample_uvc_is_need_venc(g_encoder_property[dev_no].format)) {
        return sample_send_yuv_frame(dev_no);
    }

    if (g_is_media_start[dev_no] == TD_FALSE || g_data_input_mode[dev_no] == OT_UVC_MPP_BIND_UVC ||
        (!sample_uvc_is_need_venc(g_encoder_property[dev_no].format))) {
        return TD_SUCCESS;
    }
    ...
    ret = ss_mpi_venc_get_stream(g_media_cfg[dev_no].venc_chn, &stream, 100); /* get stream timeout 100 ms */
    if (ret != TD_SUCCESS) {
        sample_print("ss_mpi_venc_get_stream failed with %#x!\n", ret);
        goto end;
    }

    if (g_data_input_mode[dev_no] == OT_UVC_SEND_VENC_STREAM) {
        ret = ss_mpi_uvc_send_stream(g_media_cfg[dev_no].dev_no, g_media_cfg[dev_no].venc_chn, &stream, 0);
        if (ret != TD_SUCCESS) {
            sample_print("ss_mpi_uvc_send_stream failed with %#x!\n", ret);
            goto end;
        }
    } else { /* OT_UVC_SEND_USER_STREAM */
        sample_uvc_send_user_stream((ot_uvc_chn)dev_no, &stream);
    }
