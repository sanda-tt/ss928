...
{
    printf("usage : %s <index> <rtsp_enable> [save_heif] [sns_type]\n", s_prg_nm);
    printf("index:\n");
    printf("\t  0) normal           :H.265e@4K@30fps + h264e@1080P@30fps.\n");
    printf("\t  1) qpmap            :H.265e@4K@30fps + h264e@1080P@30fps.\n");
    printf("\t  2) intra_refresh    :H.265e@4K@30fps(row) + h264e@1080P@30fps(column).\n");
    printf("\t  3) roi_bg_frame_rate:H.265e@4K@30fps + H.264@1080P@30fps.\n");
    printf("\t  4) debreath_effect  :H.265e@4K@30fps(enable) + H.264e@1080P@30fps(disable).\n");
    printf("\t  5) roi_set          :Mjpege@4K@30fps(enable) + Mjpege@1080P@30fps(disable).\n");
    printf("\t  6) roimap           :Mjpege@4K@30fps(enable) + Mjpege@1080P@30fps(disable).\n");
    printf("rtsp_enable:\n");
    printf("\t  0) disable rtsp : venc encode to local file (recording).\n");
    printf("\t  1) enable rtsp  : venc encode to rtsp stream (no local recording).\n");
    printf("save_heif: (optional)\n");
...
static sample_sns_type g_sns_type[4] = {
    SENSOR0_TYPE,
    SENSOR1_TYPE,
    SENSOR2_TYPE,
    SENSOR3_TYPE
...
{
    td_s32 ret;
    sample_sns_type sns_type = g_sns_type[0];
...
#endif
{
    td_s32 ret, push_rtsp = 0;
    td_u32 index;

    if (argc < 0x3 || argc > 0x5) { /* 2:arg num */
    ...
    if (argc >= 0x3) {
        push_rtsp = atoi(argv[2]);
        if (push_rtsp != 0 && push_rtsp != 1) {
            sample_venc_usage(argv[0]);
            return TD_FAILURE;
        }
        sample_comm_venc_set_push_rtsp((push_rtsp == 1) ? TD_TRUE : TD_FALSE);
        const char *rtsp_ip = "0.0.0.0";
        rtsp_server_start(rtsp_ip, 554);
        //sample_venc_set_video_param 参数
        rtsp_session_create(0, 1);  //ven0 h265
        rtsp_session_create(1, 0);  //ven1 h264
    }

    if (argc >= 0x4) {
        td_s32 save_heif = atoi(argv[3]);
    ...
    if (argc == 0x5) {
        ...
        sns_type = (sample_sns_type)atoi(argv[4]);
        ...
        g_sns_type[2] = sns_type;
        g_sns_type[3] = sns_type;
    }
