...
int H265File::ReadFrame(char* in_buf, int in_buf_size, bool* end)
{
    ...
    *end = false;
    ...
    for (i = 0; i < bytes_read - 6; i++) {
        ...
        else if(m_buf[i] == 0 && m_buf[i+1] == 0 &&
                m_buf[i+2] == 0 && m_buf[i+3] == 1) {
            start_code = 4;
        }
        else {
            continue;
        ...
        uint8_t nal_type = (m_buf[i + start_code] >> 1) & 0x3F;

        /* VCL NAL：IDR / 非 IDR / 参数集*/
        if(nal_type == 19 || nal_type == 20 ||  /* IDR */
           nal_type == 32 || nal_type == 33 || nal_type == 34 ||  /* VPS/SPS/PPS */
           (nal_type >= 0 && nal_type <= 31)) { /* 普通帧 */
            is_find_start = true;
            break;
        }
    }
    ...
    /* ---------- 查找下一帧起始，作为当前帧结束 ---------- */
    for (i += start_code; i < bytes_read - 6; i++) {
        if(m_buf[i] == 0 && m_buf[i+1] == 0 && m_buf[i+2] == 1) {
            start_code = 3;
        }
        else if(m_buf[i] == 0 && m_buf[i+1] == 0 &&
