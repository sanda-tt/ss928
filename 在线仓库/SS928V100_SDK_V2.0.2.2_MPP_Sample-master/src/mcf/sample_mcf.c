...
static td_void sample_mcf_imx347_cfg(const ot_mcf_grp_attr *mcf_grp_attr)
{
...
static td_s32 sample_mcf_path_switch_sync_true(ot_mcf_grp mcf_grp)
{
    ...
    sample_get_char("switch to color path!-------- --sync: TRUE-----------");
    ret = sample_mcf_set_path(mcf_grp, OT_MCF_PATH_COLOR);
    if (ret != TD_SUCCESS) {
        return ret;
    }

    sample_get_char("switch to mono path!------- - --sync: TRUE-----------");
    ret = sample_mcf_set_path(mcf_grp, OT_MCF_PATH_MONO);
    if (ret != TD_SUCCESS) {
        return ret;
    }

    sample_get_char("switch to fusion path!----------sync: TRUE-----------");
    ret = sample_mcf_set_path(mcf_grp, OT_MCF_PATH_FUSION);
    if (ret != TD_SUCCESS) {
        return ret;
    }

    sample_get_char("swith to mono path!-------------sync: TRUE-----------");
    ret = sample_mcf_set_path(mcf_grp, OT_MCF_PATH_MONO);
    if (ret != TD_SUCCESS) {
        return ret;
    }
    ...
    sample_vi_cfg *vi_cfg0, sample_vi_cfg *vi_cfg1)
{
    ...
    ret = sample_mcf_fusion_to_color_path(mcf_grp, mcf_grp_attr, vi_cfg0);
    if (ret != TD_SUCCESS) {
        return ret;
    }

    sample_get_char("switch to color to mono path!--------  --sync: FALSE-----------");
    ret = sample_mcf_color_to_mono_path(mcf_grp, mcf_grp_attr, vi_cfg0, vi_cfg1);
    if (ret != TD_SUCCESS) {
        return ret;
    }

    sample_get_char("switch to mono to fusion path!----------sync: FALSE-----------");
    ret = sample_mcf_mono_to_fusion_path(mcf_grp, mcf_grp_attr, vi_cfg1);
    if (ret != TD_SUCCESS) {
        return ret;
    }

    sample_get_char("switch to fusion to mono path!----------sync: FALSE-----------");
    ret = sample_mcf_fusion_to_mono_path(mcf_grp, mcf_grp_attr, vi_cfg1);
    if (ret != TD_SUCCESS) {
        return ret;
    }

    sample_get_char("switch to mono to color path!----------sync: FALSE-----------");
    ret = sample_mcf_mono_to_color_path(mcf_grp, mcf_grp_attr, vi_cfg0, vi_cfg1);
    if (ret != TD_SUCCESS) {
        return ret;
    }

    sample_get_char("switch to color to fusion path!----------sync: FALSE-----------");
    ret = sample_mcf_color_to_fusion_path(mcf_grp, mcf_grp_attr, vi_cfg0);
    if (ret != TD_SUCCESS) {
        return ret;
    }
    return ret;
    ...
    sample_vi_cfg *vi_cfg0, sample_vi_cfg *vi_cfg1)
{
    ...
    if (mcf_grp_attr->sync_pipe == TD_TRUE) {
        ret = sample_mcf_path_switch_sync_true(mcf_grp);
        if (ret != TD_SUCCESS) {
            return ret;
        }
    } else {
        ret = sample_mcf_path_switch_sync_false(mcf_grp, mcf_grp_attr, vi_cfg0, vi_cfg1);
        if (ret != TD_SUCCESS) {
            return ret;
        }
    }
...
static td_s32 sample_mcf_switch(td_bool sync_enable)
{
    ...
    mcf_grp_attr.sync_pipe = sync_enable;
    yuv_cnt.color_cnt = VB_MCF_YUV_CNT;
    yuv_cnt.mono_cnt = VB_MCF_YUV_CNT;
    ret = sample_mcf_start_mcf_data_flow(mcf_grp, &mcf_grp_attr, large_size, &grp_crop, yuv_cnt);
    if (ret != TD_SUCCESS) {
...
static td_void sample_mcf_usage(char *prg_name)
{
    ...
    sample_print("    (3) mcf switch with sync      : mcf switch to daytime and night without destory\n");
    sample_print("    (4) mcf switch without sync   : mcf switch to daytime and night with destory\n");
    sample_print("    (5) mcf correction route      : mcf correct with image of equal size, "
...
static td_s32 sample_mcf_execute_case(td_u32 case_index)
{
    ...
    switch (case_index) {
        ...
        case 4: /* 4 mcf switch to daytime and night with destory. */
            ...
            break;
        case 5: /* 5 mcf with input image of equal size , and do correction. */
