...
const static td_bool g_venc_bind_uvc_flag = TD_FALSE;
...
td_s32 sample_venc_get_uvc_send(td_void)
{
    ...
    td_s32 timeout = 100; /* get stream timeout 100 ms */

    if (g_venc_bind_uvc_flag || (!sample_uvc_is_need_venc(g_encoder_property.format))) {
        return TD_SUCCESS;
    }
    ...
    ret = ss_mpi_venc_get_stream(g_media_cfg.venc_chn, &stream, timeout);
    if (ret != TD_SUCCESS) {
