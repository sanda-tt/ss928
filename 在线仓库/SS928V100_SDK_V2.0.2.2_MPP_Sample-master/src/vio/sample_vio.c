...
static sample_vo_cfg g_vo_cfg = {
    ...
    .intf_sync         = OT_VO_OUT_1080P60, //OT_VO_OUT_1080P30,
...
static td_void sample_vi_get_two_sensor_vi_cfg(sample_sns_type sns_type, sample_vi_cfg *vi_cfg0, sample_vi_cfg *vi_cfg1)
{
    ...
    vi_cfg0->sns_info.bus_id = 5; /* i2c5 */

    vi_cfg1->sns_info.bus_id = 7; /* i2c7 */
...
static td_void sample_vi_get_four_vi_cfg(sample_sns_type sns_type, sample_vi_cfg *vi_cfg, td_s32 dev_num)
{
    ...
    vi_cfg[0].sns_info.bus_id = 7; /* arg0: i2c7 */
    vi_cfg[1].sns_info.bus_id = 5; /* arg1: i2c5 */
    vi_cfg[2].sns_info.bus_id = 4; /* arg2: i2c4 */
    vi_cfg[3].sns_info.bus_id = 6; /* arg3: i2c6 */
}

/* 
    1路 4lane sensor 

    EULER_2R J3 sensor0 ---> i2c5 clk0
...
static td_s32 sample_vio_one_sensor(int sensor_num)
{
    td_s32 ret;
    ...
    ret = sample_vio_sys_init(mode_type, video_mode, VB_YUV_ROUTE_CNT, VB_RAW_CNT_NONE);
    if (ret != TD_SUCCESS) {
        goto sys_init_failed;
    }

    sample_vi_get_one_sensor_vi_cfg(sns_type, &vi_cfg[0], sensor_num);
    ret = sample_vio_start_multi_vi_vpss(vi_cfg, vpss_grp, 1, 1);/* start 1 route */
    if (ret != TD_SUCCESS) {
        goto start_vi_vpss_failed;
    }
...
/* 
    ...
    EULER_2R V1.0 J3 sensor0 ---> i2c5 clk0
                  J4 sensor1 ---> i2c7 clk1

    已适配 imx347  4lane x 2
    已适配 os08a20 4lane x 2
    已适配 os04a10 4lane x 2
*/
...
/* Euler sample_vio 1/2/4 sensor 适配sensor如下：

   imx347 4lane sensor0/1
   imx347 4lane sensor0 + sensor1
   imx347 2lane sensor0 + sensor1 + sensor2 + sensor3

   os08a20 4lane sensor0/1
   os08a20 4lane sensor0 + sensor1

   os04a10 4lane sensor0/1
   os04a10 4lane sensor0 + sensor1
*/

static td_void sample_vio_usage(char *prg_name)
{
    printf("usage : %s <index> <venc_en>\n", prg_name);
    printf("index:\n");
    printf("    (0) one  sensor     4lane sensor0              :vi one  sensor (Online)  -> vpss -> venc && vo.\n");
    printf("    (1) one  sensor     4lane sensor1              :vi one  sensor (Online)  -> vpss -> venc && vo.\n");
    printf("    (2) two  sensor     4lane sensor(0~1)          :vi two  sensor (offline) -> vpss -> venc && vo.\n");
    printf("    (3) four sensor     2lane sensor(0~3)          :vi four sensor (offline) -> vpss -> venc && vo.\n");
    printf("venc_en:\n");
    printf("    (0) Disable VENC (Video Encoding)\n");
    printf("    (1) Enable VENC (Video Encoding)\n");
}
...
static td_s32 sample_vio_execute_case(td_u32 case_index)
{
    ...
    switch (case_index) {
        case 0: /* 0 one sensor */
            ret = sample_vio_one_sensor(0);
            break;
        case 1: /* 1 one sensor */
            ret = sample_vio_one_sensor(1);
            break;
        case 2: /* 2 two sensor */
            ret = sample_vio_two_sensor();
            break;
        case 3: /* 3 four sensor */
