...
extern int PQStreamMain(int argc, char **argv);

static void *PQStreamMainThread(void *arg)
{
    int argNum = 3;
    char *args[] = { "", "hy_s0603", "0"};
    PQStreamMain(argNum, args);
    return NULL;
}
...
int main(int argc, char* argv[])
{
    ...
    printf("stream_thread start\n");
    pthread_create(&streamThreadId, NULL, PQStreamMainThread, NULL);
    printf("stream_thread end\n");
    ...
    (td_void)getchar();
    if (pthread_kill(streamThreadId, SIGSYS) != 0) {
        perror("pthread_kill");
    }
    pthread_join(streamThreadId, NULL);
