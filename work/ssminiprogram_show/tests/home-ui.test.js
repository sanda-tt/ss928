...
test("home page hides demo-only SmartBag debug entry", () => {
  assert.strictEqual(homeWxml.indexOf("smartbag-debug-entry"), -1);
  assert.strictEqual(homeWxml.indexOf("SS928-SmartBag 调试入口"), -1);
  assert.strictEqual(homeWxml.indexOf("调试入口"), -1);
  assert.strictEqual(homeWxss.indexOf("smartbag-debug-entry"), -1);
});
