...
本项目目标是比赛演示，不是商业产品开发。所有开发任务遵循：

1. 优先实现最小可运行版本（MVP），优先保证功能跑通，不主动增加复杂架构。
2. 不设计商业级方案，例如用户系统、权限、安全认证、复杂云架构、复杂数据库设计或容灾，除非用户明确要求。
3. 不因为“未来可能需要”而提前扩展功能，禁止主动扩大需求范围。
4. 遇到需求时，先给出并实施最快可行方案，再按用户要求考虑优化方案。
5. 默认选择最少代码、最少依赖、最少组件和最少配置步骤的实现。
6. 发现潜在问题时，只需用一句话提醒风险和简单解决措施，不展开复杂设计，不输出大量安全、工程规范或商业化建议。

以上原则用于约束实现范围和方案复杂度，不替代用户当前明确要求，也不削弱下述硬件操作、凭据和原始资料安全边界。
