...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--side", choices=("left", "right"), default="left", help="Camera side used in emitted alert JSONL.")
    parser.add_argument("--emit-alert-jsonl", action="store_true", help="Emit one JSONL vision_alert event for the highest active warning.")
...
def select_highest_risk_assessment(
    ...
    if not warnings:
        return None
    return max(warnings, key=lambda item: (int(item.level), item.score, item.track_id))


def build_vision_alert_event(
    side: str,
    assessment: RiskAssessment | None,
    timestamp_s: float,
) -> dict[str, object] | None:
    if assessment is None or assessment.level <= RiskLevel.SAFE:
        return None
    ...
    return {
        ...
        "track_id": int(assessment.track_id),
        "ts": round(float(timestamp_s), 3),
    }
    ...
    while True:
        ...
        if not paused:
            ...
            if args.emit_alert_jsonl:
                alert_event = build_vision_alert_event(
                    args.side,
                    select_highest_risk_assessment(risk_by_track_id),
                    timestamp_s,
                )
                if alert_event is not None:
                    print(json.dumps(alert_event, separators=(",", ":")), flush=True)
